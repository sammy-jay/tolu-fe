...
function SideBar() {
  ...
  async function getResponse(param) {
    ...
    if (param === "Customers") {
        // alert("C")
         try {
      response = await fetch("http://localhost:3000/itrack/customers");
      ...
      setNavRes({ ...navRes, customers: data.message });
        ...
        // alert("T")
        try {
            response = await fetch("http://localhost:3000/itrack/transactions");
            data = await response.json();
            setLoadingState(false);
  ...
  //  alert(JSON.stringify(customerDetails))
    let response = await fetch("http://localhost:3000/itrack/create-customer", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
